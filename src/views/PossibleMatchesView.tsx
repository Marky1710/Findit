...
import { findMatches, getMatchStrengthDetails } from '../utils/matchingAlgorithm';
...
export const PossibleMatchesView: React.FC = () => {
  ...
  const matchPairings = useMemo(() => {
    ...
    const activeFound = items.filter(item => !item.deleted && item.type === 'FOUND' && item.status !== 'RECOVERED');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-bold mb-2 border border-blue-200">
            ...
            <span>Automated Lost-Found Matching Engine</span>
          </div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight font-sans">
            Possible Matches
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Automated pairings between reported Lost and Found items evaluated against our 100-point scoring algorithm.
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
                  ...
                  {/* 5-parameter checkmark indicators */}
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                    <span className={`px-2 py-0.5 rounded-full flex items-center space-x-1 font-medium ${
                      matchDetails.categoryScore >= 20 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-400'
                    }`}>
                      <Check className="w-3 h-3" />
                      <span>Category ({matchDetails.categoryScore}/30)</span>
                    </span>
                    ...
                    <span className={`px-2 py-0.5 rounded-full flex items-center space-x-1 font-medium ${
                      matchDetails.dateScore >= 15 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-400'
                    }`}>
                      <Check className="w-3 h-3" />
                      <span>Date ({matchDetails.dateScore}/20)</span>
                    </span>

                    <span className={`px-2 py-0.5 rounded-full flex items-center space-x-1 font-medium ${
                      matchDetails.colorScore >= 12 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-400'
                    }`}>
                      <Check className="w-3 h-3" />
                      <span>Color ({matchDetails.colorScore}/15)</span>
                    </span>

                    <span className={`px-2 py-0.5 rounded-full flex items-center space-x-1 font-medium ${
                      matchDetails.keywordsScore >= 4 ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-400'
                    }`}>
                      <Check className="w-3 h-3" />
                      <span>Keywords ({matchDetails.keywordsScore}/10)</span>
                    </span>
                  </div>
                </div>
