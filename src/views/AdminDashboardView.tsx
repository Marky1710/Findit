...
export const AdminDashboardView: React.FC = () => {
  ...
  const executeDeleteUser = async (target: User) => {
    ...
    if (result.success) {
      ...
      setActionFeedback(`User ${target.name} was successfully deleted.`);
  ...
  const handleDeleteUserClick = async (targetUser: User) => {
    let confirmedByWindow = false;
    try {
      if (typeof window !== 'undefined' && typeof window.confirm === 'function') {
        confirmedByWindow = window.confirm(`Are you sure you want to delete the user ${targetUser.name}? This action cannot be undone and will also delete their items.`);
      }
    } catch {
      confirmedByWindow = false;
    }

    if (confirmedByWindow) {
      await executeDeleteUser(targetUser);
      return;
    }
                      ...
                      {user.role !== 'admin' && (
                        <>
                          ...
                          <button
                            ...
                            className="px-2 py-1 bg-red-600 hover:bg-red-700 text-white font-bold rounded text-[11px] ml-2 transition-colors cursor-pointer"
                          >
                            Delete
                          </button>
                        </>
...
{`CREATE TABLE activity_logs (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      ...
      {userToDelete && (
        <div id="delete-user-modal-overlay" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-xs p-4 animate-in fade-in duration-150">
          <div id="delete-user-modal" className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  Are you sure you want to delete the user <strong className="text-slate-900">{userToDelete.name}</strong> ({userToDelete.studentId || userToDelete.email})?
                </p>
                <p className="text-[11px] text-red-600 font-medium pt-1">
                  This action cannot be undone and will deactivate their account, invalidate active sessions, and remove their posted listings.
                </p>
              </div>
            </div>

            {deleteUserError && (
              <div id="delete-user-error-msg" className="p-3 bg-red-50 border border-red-200 text-red-700 text-xs rounded-xl font-medium">
                {deleteUserError}
              </div>
            )}
            ...
            <div className="flex justify-end space-x-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                disabled={isDeletingUser}
                className="px-4 py-2 text-xs font-bold bg-red-600 hover:bg-red-700 text-white rounded-xl shadow-xs transition-colors flex items-center space-x-1.5 cursor-pointer"
              >
                {isDeletingUser ? (
                  <>
